...
function addFormValidate() {
	...
	function check_serviceType() {
		...
		if (date != '') {
			...
			$("#date_error_message").html("<i style='color:red;font-size: 0.9em'>Past date!</i>"+date);
		
		}else if(date === '03/30/2021'){
			
			$("#date_error_message").html("<i style='color:red;font-size: 0.9em'>Past date!</i>");
			...
			$("#date_error_message").html("<i style='color:red;font-size: 0.9em'>Please select a valid date!</i>"+date);
			$("#date_error_message").show();
			$("#datepicker-autoclose").css("border-bottom", "2px solid #F90A0A");
			error_date = true;
		}
...
function editFormValidate() {
	...
	function check_serviceType() {
		...
		m = m > 9 ? m : "0"+m;
		d = d > 9 ? d : "0"+d;
		var prettyDate = m +'/'+ d +'/'+ myDate.getFullYear();
		*/
		
		var date = $("#datepicker-autoclose").val(); 
		
		if (date != '' && date < future_date) {
			$("#date_error_message").hide();
			$("#datepicker-autoclose").css("border-bottom", "2px solid #34F458");
